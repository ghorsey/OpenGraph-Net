...
<body>
    ...
    public async Task Test301RedirectUrl()
    {
        ...
        {
            Assert.Fail($"Got an exception for a 301 redirect: {ex.Message}");
        }
    }
    ...
    /// <returns>A task.</returns>
    [Fact]
    public async Task TestMetaCharsetParsesCorrectly()
    {
        ...
        var ogs = await OpenGraph.ParseUrlAsync("http://www.telerama.fr/cinema/realite-virtuelle-360-de-bonheur-a-ameliorer,144339.php?utm_medium=Social&utm_source=Twitter&utm_campaign=Echobox&utm_term=Autofeed#link_time=1466595239");
    ...
    {
        await Assert.ThrowsAsync<HttpRequestException>(() => OpenGraph.ParseUrlAsync("https://github.com/ghorsey/OpenGraph-Net/i-dont-exist"));
    }
    ...
    /// <returns>A task.</returns>
    [Fact]
    public async Task TestParsingA404Url()
    {
        await Assert.ThrowsAsync<HttpRequestException>(() => OpenGraph.ParseUrlAsync("https://github.com/ghorsey/OpenGraph-Net/i-dont-exist"));
    }
    ...
    {
        await Assert.ThrowsAsync<HttpRequestException>(() => OpenGraph.ParseUrlAsync("https://www.developors.facebook.com/"));
    }
    ...
    /// <returns>A task.</returns>
    [Fact]
    public async Task TestParsingABadHostUrl()
    {
        await Assert.ThrowsAsync<HttpRequestException>(() => OpenGraph.ParseUrlAsync("https://www.developors.facebook.com/"));
    }
    ...
    /// <returns>A task.</returns>
    [Fact]
    public async Task TestIssue31FromGit()
    {
        var url = "https://www.nasdaq.com/boardvantage/board-portal";
        var og = await OpenGraph.ParseUrlAsync(url);
        Assert.Equal("Solutions 2021", og.Type);
    }
